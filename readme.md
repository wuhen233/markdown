...
语法：
[![github](https://img.shields.io/badge/github-wuhen233-brightgreen.svg)](https://github.com/wuhen233)
```
...
```
语法：[^变量]在文档结尾或其他位置給变量赋值：[^变量]: 注明框内显示的内容鼠标移到这里> [^哈喽][^哈喽]: 注明框内显示的内容
```
...
```
语法：<details><summary>点我打开关闭折叠</summary> 折叠内容 <ul>     <li>1</li>     <li>2</li>     <li>3</li> </ul></details>
```
...
```
语法：1. 第一项：    - 第一项嵌套的第一个元素    - 第一项嵌套的第二个元素2. 第二项：    - 第二项嵌套的第一个元素    - 第二项嵌套的第一个元素
```
...
```
语法：> 区块中使用列表> 1. 第一项> 2. 第二项> * 111> * 222
```
...
```
语法：​```javascriptfunction test() {	alert('test')}​```
```
...
```
语法：[链接名称](链接地址)[链接名称](链接地址,可选的alt)或<链接地址>
```

例如：

```
语法：[百度](http://www.baidu.com,'百度')
```
...
```
语法：链接可以用变量来代替，文档末尾或其他位置附带变量地址：这个链接用 1 作为网址变量 [Google][1]这个链接用 baidu 作为网址变量 [Baidu][baidu]然后在文档的结尾为变量赋值（网址）  [1]: http://www.google.com/  [baidu]: http://www.baidu.com/
```
...
```
语法：可使用相对路径（前提是有该路径下的文件）[test](test.md)
```
...
```
语法：![alt替代文本](图片地址)![alt替代文本](图片地址 "title标题")
```
...
```
这里链接用 img 作为图片地址变量 然后在文档的结尾或其他位置给变量赋值（图片地址）语法：![RUNOOB][img][img]: ./test/logo.png
```
...
```
语法：<img src="./test/logo.png" width="50px" height="30px">
```
...
```
语法：![其他仓库的图片](https://gitee.com/wuhen233/pic/raw/master/mypictures/test.jpg)
```



![其他仓库的图片](https://gitee.com/wuhen233/pic/raw/master/mypictures/test.jpg)
...
```
语法：| 左对齐 | 右对齐 | 居中对齐 || :-----| ----: | :----: || 单元格 | 单元格 | 单元格 || 单元格 | 单元格 | 单元格 |
```
...
```
语法：  Emoji表情英文名的前后加冒号，Typore上先输入冒号再输入首字母有表情提示如：:smirk:
```
...
```
语法：使用 <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>Del</kbd> 重启电脑
```
...
```
语法：$$\mathbf{V}_1 \times \mathbf{V}_2 =  \begin{vmatrix} \mathbf{i} & \mathbf{j} & \mathbf{k} \\\frac{\partial X}{\partial u} &  \frac{\partial Y}{\partial u} & 0 \\\frac{\partial X}{\partial v} &  \frac{\partial Y}{\partial v} & 0 \\\end{vmatrix}$$tep1}{\style{visibility:hidden}{(x+1)(x+1)}}$$
```
...
```
语法：​```chart,Budget,Income,Expenses,DebtJune,5000,8000,4000,6000July,3000,1000,4000,3000Aug,5000,7000,6000,3000Sep,7000,2000,3000,1000Oct,6000,5000,4000,2000Nov,4000,3000,5000,type: pietitle: Monthly Revenuex.title: Amounty.title: Monthy.suffix: $​```​```mermaidsequenceDiagramA->>B: 是否已收到消息？B-->>A: 已收到消息​```
```

> 注：在Typora中未支持

```chart
,Budget,Income,Expenses,DebtJune,5000,8000,4000,6000July,3000,1000,4000,3000Aug,5000,7000,6000,3000Sep,7000,2000,3000,1000Oct,6000,5000,4000,2000Nov,4000,3000,5000,type: pietitle: Monthly Revenuex.title: Amounty.title: Monthy.suffix: $
```
...
```
语法：​```mermaidgraph TDA[模块A] -->|A1| B(模块B)B --> C{判断条件C}C -->|条件C1| D[模块D]C -->|条件C2| E[模块E]C -->|条件C3| F[模块F]​```
```
...
```
语法：​```mermaidsequenceDiagramA->>B: 是否已收到消息？B-->>A: 已收到消息​```
```
...
``` ···
语法：​```mermaidgantttitle 甘特图dateFormat  YYYY-MM-DDsection 项目A任务1           :a1, 2018-06-06, 30d任务2     :after a1  , 20dsection 项目B任务3      :2018-06-12  , 12d任务4      : 24d​```
```
